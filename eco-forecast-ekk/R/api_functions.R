# =============================================================================
# api_functions.R — Veri Kapısı (Dış Dünyayla Konuşan Tek Katman)
# -----------------------------------------------------------------------------
# Dış dünya güvenilmezdir: bağlantı kopar, servis çöker, veri bozuk gelir.
# Bu kaosu YOK EDEMEYİZ; ama tek bir odaya hapsedip kapıyı kapatabiliriz.
# Projenin geri kalanı buraya sadece "bana şu göstergeyi ver" der.
#
# ORTAK KURAL: Her tercüman, kaynaktan ne biçimde alırsa alsın, HEP aynı üç
# sütunla döner — tarih, deger, kimlik. Farklılık kaynakta kalır.
#
# ÖNERİ İLE UYUM:
#   Risk 1 (API kesintisi) -> fetch_indicator: birincil kaynak çökerse yedek
#     kaynağa otomatik geçiş, ardından bayat cache, ardından CSV yedek.
#   Risk 2 (veri kalitesi) -> her çekim için metaveri (kaynak, kod, tarih
#     aralığı, çekim zamanı, yedek kullanıldı mı) kaydedilir.
#
# Yükleme sırası: config.R -> utils.R -> api_functions.R -> ...
# =============================================================================
...
# --- Paketler -----------------------------------------------------------------
# install.packages(c("fredr", "httr", "jsonlite", "wbstats"))
...
#' Dönem metnini tarihe çevir
#'
#' EVDS ve OECD sıklığa göre farklı dönem yazar; hepsi dönemin İLK gününe
#' çevrilir: günlük \code{31-12-2015}, aylık \code{2015-1} / \code{2015-01},
#' çeyreklik \code{2015-Q1}, yıllık \code{2015}. Tanınmayan biçim \code{NA}
#' olur (sessizce yanlış tarih üretmemek için).
#'
#' @param x Dönem metinleri.
...
#' Standart üç sütunlu tabloya çevir
#'
#' NA değerleri atar, aynı tarihte tekrarı tekilleştirir, tarihe göre sıralar
#' ve config'teki tarih aralığına keser.
#'
#' @param tarih Tarih vektörü.
#' @param deger Sayısal değer vektörü.
#' @param kimlik Gösterge adı.
#' @return \code{data.frame(tarih, deger, kimlik)}.
...
#' HTTP GET (ortak hata yönetimiyle)
#'
#' 400/401/403/404 kalıcı hata sayılır (tekrar denemeye değmez); diğer HTTP
#' hataları ve bağlantı kopmaları geçicidir.
#'
#' @param url Adres.
#' @param basliklar \code{httr::add_headers(...)} nesnesi ya da \code{NULL}.
#' @return Yanıt gövdesi (UTF-8 metin).
...
# =============================================================================
# 2) TERCÜMANLAR — hepsi (kod, kimlik, filtre) alır, (tarih, deger, kimlik) döner
# =============================================================================

#' Tercüman: FRED
#'
#' @param kod FRED seri kimliği.
#' @param kimlik Gösterge adı.
#' @param filtre Kullanılmaz (ortak imza için).
#' @return \code{data.frame(tarih, deger, kimlik)}.
...
#' Tercüman: TCMB EVDS
#'
#' Anahtar "key" başlığıyla gider. Kod biçimi: Excel'de/yanıtta \code{TP_FG_J0}
#' (alt çizgi), istekte \code{TP.FG.J0} (nokta); çeviri burada yapılır.
#' Adres: \code{CONFIG$baglanti$evds_base_url} (evds3). \code{filtre}, sorguya
#' eklenen ek parametredir (örn. \code{"frequency=5&aggregationTypes=avg"}: günlük
#' seriyi aylık ortalamaya topla).
#'
#' @inheritParams fred_cek
#' @return \code{data.frame(tarih, deger, kimlik)}.
...
#' Dünya Bankası kodunu temizle
#'
#' \code{WB_WDI_NY_GDP_MKTP_KD_ZG} -> \code{NY.GDP.MKTP.KD.ZG};
#' \code{SL.UEM.TOTL.ZS?locations=TR} -> \code{SL.UEM.TOTL.ZS}.
#'
#' @param kod Excel'deki kod.
#' @return \code{wbstats}'ın istediği kod.
...
#' Tercüman: Dünya Bankası (anahtar istemez, yıllık)
#'
#' @inheritParams fred_cek
#' @return \code{data.frame(tarih, deger, kimlik)}; tarih yıl başı.
...
#' Tercüman: OECD (SDMX API)
#'
#' OECD'nin eski \code{OECD} R paketi 2024'teki API değişikliğinden sonra
#' çalışmaz; yeni SDMX uç noktasına doğrudan konuşulur. Bir veri akışı tek başına
#' seri belirlemez, bu yüzden \code{filtre} (SDMX anahtarı) şarttır ve TEK bir
#' seriye indirgemelidir.
#'
#' @param kod \code{"KURUM,VERI_AKISI"}, örn. \code{"OECD.SDD.TPS,DSD_BOP@@DF_BOP"}.
#' @param kimlik Gösterge adı.
#' @param filtre SDMX anahtarı, örn. \code{"TUR.WXD.CA.B.T.Q.USD_EXC.N"}.
#' @return \code{data.frame(tarih, deger, kimlik)}.
...
#' Tercüman: Borsa İstanbul (Yahoo Finance grafik API'si, aylık kapanış)
#'
#' Öneri'nin B planı Yahoo Finance'i alternatif kaynak olarak anar. Excel'deki
#' \code{XU100} kodu Yahoo'da \code{XU100.IS} sembolüdür. Anahtar istemez.
#'
#' @param kod Excel'deki kod (örn. \code{"XU100"}) ya da tam sembol.
#' @param kimlik Gösterge adı.
#' @param filtre Kullanılmaz.
#' @return \code{data.frame(tarih, deger, kimlik)} (ay başı).
...
# =============================================================================
# 3) KAYNAK ZİNCİRİ VE YÖNLENDİRİCİ
# =============================================================================

#' Kaynak kullanılabilir mi?
#'
#' Anahtar isteyen kaynaklarda anahtar yoksa çağrı yapılmaz (boşuna beklemek ve
#' anlaşılmaz 401 hatası yerine zincirde bir sonrakine geçilir).
#'
#' @param kaynak Kaynak adı.
...
#' Yedek kaynak asıl seriyle uyumlu mu? (tür)
#'
#' Aynı isim aynı ölçü demek değildir. \code{"farkli"} etiketli (tanımı/ölçeği
#' farklı) seriler asla yedek olmaz; \code{"oran"} (zaten yüzde değişim) yalnız
#' hedefi \code{"yillik_degisim"} olan göstergelere yedek olabilir.
#'
#' @param hedef Göstergenin \code{donusum} değeri ("duzey" ya da "yillik_degisim").
#' @param tur Adayın türü ("duzey", "oran", "farkli").
#' @return \code{TRUE}/\code{FALSE}.
...
#' Kaynağın frekansı, atanan frekans için yeterince sık mı?
#'
#' Sıkı bir kaynak seyrek frekansa toplanabilir (aylık -> çeyreklik); seyrek bir
#' kaynak sıkı frekansa dönüştürülemez (yıllık -> aylık uydurma olurdu).
#'
#' @param hedef Göstergenin atanan frekansı.
#' @param kaynak_frekans Adayın frekansı.
#' @return \code{TRUE}/\code{FALSE}.
...
#' Bir göstergenin kaynak zinciri (KENDİ öncelik sırasıyla)
#'
#' Sıra kartın \code{oncelik} alanından gelir: 1. ASIL kaynak, 2. BİRİNCİL
#' YEDEK, sonrakiler sıradaki yedekler; \code{"CSV_YEDEK"} yerel yedek kanaldır.
#' Yedekler tür, frekans, filtre ve tercüman uyumuna göre elenir (uyumsuz yedek
#' zincire girmez). \code{yedek_kaynak_kullan = FALSE} ise yalnız asıl kaynak.
#'
#' @param gosterge_adi Gösterge adı.
#' @return Liste; her eleman \code{list(kaynak, kod, filtre, tur, carpan, yaklasik, birincil)}.
...
#' Seriyi atanan frekansa getir
#'
#' Tarihleri dönem başına yuvarlar ve aynı döneme düşen değerleri ortalar
#' (günlük -> aylık, aylık -> çeyreklik gibi). Zaten o frekanstaysa değişmez.
#'
#' @param d \code{data.frame(tarih, deger, kimlik)}.
#' @param frekans Hedef frekans.
#' @return \code{data.frame(tarih, deger, kimlik)}.
...
#' Seriyi hedef biçime dönüştür
#'
#' \code{carpan} birim farkını giderir. Hedef \code{"yillik_degisim"} ve kaynak
#' \code{"duzey"} ise her gözlem, bir yıl öncesinin AYNI gününe göre yüzde değişime
#' çevrilir (aylık, çeyreklik ve yıllık serilerde aynı kod çalışır; bir yıl
#' öncesi yoksa gözlem düşer). Kaynak zaten \code{"oran"} ise dokunulmaz.
#'
#' @param d \code{data.frame(tarih, deger, kimlik)}.
#' @param hedef "duzey" ya da "yillik_degisim".
#' @param tur Kaynağın verdiği biçim ("duzey" ya da "oran").
#' @param carpan Değer çarpanı.
#' @return Dönüştürülmüş \code{data.frame(tarih, deger, kimlik)}.
...
#' Kaynağa göre doğru tercümanı çağır
#'
#' @param kaynak Kaynak adı (FRED, EVDS, WORLDBANK, OECD, BIST, CSV_YEDEK).
#' @param kod Kaynağın seri kodu.
#' @param kimlik Gösterge adı.
#' @param filtre OECD SDMX anahtarı ya da EVDS sorgu eki (yoksa \code{NULL}).
#' @return \code{data.frame(tarih, deger, kimlik)}.
...
#' Çekilmiş veriye metaveri iliştir
#'
#' @param veri \code{data.frame(tarih, deger, kimlik)}.
#' @param kaynak,kod,filtre,birincil Kullanılan zincir halkasının bilgileri.
#' @param durum "api", "cache", "bayat_cache" ya da "csv_yedek".
#' @param cekim_zamani Çekim zamanı (\code{POSIXct}).
#' @param donusum Uygulanan dönüşüm ("duzey" ya da "yillik_degisim").
#' @param tur Kaynağın verdiği biçim.
#' @param frekans İşlenen frekans.
#' @param yaklasik Yedek seri asıl seriye yakın ama aynı tanımda değilse \code{TRUE}.
#' @return \code{veri}, \code{"meta"} özniteliğiyle.
...
#' Bir göstergeyi getir (PROJENİN KALBİ)
#'
#' Sıra: (1) taze cache; (2) kaynak zinciri — göstergenin KENDİ öncelik sırası:
#' asıl kaynak, birincil yedek, sonraki yedekler, CSV yedek kanalı (her API
#' \code{tekrar_dene} ile); (3) bayat cache. Çekilen seri önce ATANAN FREKANSA
#' getirilir, sonra hedef biçime (düzey / yıllık % değişim) dönüştürülür.
#' Başarılı API çekimi cache'e ve CSV yedeğe yazılır. Kullanılan kaynak
#' \code{attr(., "meta")} içindedir.
#'
#' @param gosterge_adi Gösterge adı (config'teki anahtar).
#' @param tazelik_gun Cache tazeliği; boşsa \code{CONFIG$veri$cache_tazelik_gun}.
#' @param yenile \code{TRUE} ise cache atlanır, doğrudan kaynaklara gidilir.
#' @return \code{data.frame(tarih, deger, kimlik)} ya da \code{NULL}.
fetch_indicator <- function(gosterge_adi, tazelik_gun = NULL, yenile = FALSE) {
  ...
  # 1) Taze cache. Kod, dönüşüm ya da frekans config'te değiştiyse eski cache geçersiz.
  if (!yenile) {
  ...
  # 2) Kaynak zinciri: bu verinin KENDİ öncelik sırası
  for (halka in kaynak_zinciri(gosterge_adi)) {
    ...
    # CSV yedek zaten hedef biçimdedir (başarılı çekimden sonra o biçimde saklanır)
...
# =============================================================================
# 4) TOPLU ÇEKİM VE METAVERİ
# =============================================================================

#' Metaveri tablosu oluştur
#'
#' Öneri (2.8): "Her veri seti için metaveri kaydı tutulacak ve veri güncellenme
#' zamanları izlenecektir." Çekilemeyen göstergeler de \code{durum = "alinamadi"}
#' olarak listelenir.
#'
#' @param sonuclar Gösterge adıyla adlandırılmış liste (elemanlar veri ya da NULL).
#' @return data.frame: gösterge, ad, rol, frekans, kaynak, kod, birincil_kaynak,
#'   birincil_yedek, kaynak_onceligi, donusum, yaklasik, yedek_kullanildi, durum,
#'   ilk_tarih, son_tarih, gozlem, cekim_zamani.
...
#'
#' Biri gelmezse ATLANIR, gerisi tek tabloda birleştirilir ("eldekini koru").
#' Metaveri \code{data/processed/metaveri.(rds|csv)} olarak kaydedilir ve
#' sonucun \code{"metaveri"} özniteliğinde de döner.
#'
#' @param yenile \code{TRUE} ise cache atlanır.
#' @return Uzun biçimli \code{data.frame(tarih, deger, kimlik)} ya da \code{NULL}.
...
#' Bir göstergenin elimizdeki en yeni tarihi
#'
#' @param veri Uzun biçimli tablo.
#' @param gosterge_adi Gösterge adı.
#' @return \code{Date}; veri yoksa \code{NA}.
...
local({
  ...
  # Her aktif verinin yedek kanalı gerçekten ÇALIŞABİLİR olmalı ve tanımlı BİRİNCİL
  # YEDEK, zincirin 2. halkası olmalı (uyumsuz tür/frekans yüzünden elenmemiş).
